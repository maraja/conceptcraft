...
(function () {
  ...
  var STATIONS = [
    { id: 'st0', th: 0,   no: '01', name: 'INGEST',            plain: 'choose what it sees',
      lx: 0,  ly: 48,  la: 'middle', pAbove: false },  // feeders above, labels below
    { id: 'st1', th: 60,  no: '02', name: 'ASSEMBLE',          plain: 'order the window',
      lx: 36, ly: -27, la: 'start',  pAbove: true },   // stack inside-ring, labels up-right
    { id: 'st2', th: 120, no: '03', name: 'MODEL CALL',        plain: 'the one line you rent',
      lx: 36, ly: 35,  la: 'start',  pAbove: false },  // box up-right, labels down-right
    { id: 'st3', th: 180, no: '04', name: 'PARSE + ROUTE',     plain: 'text becomes action',
      lx: 0,  ly: -48, la: 'middle', pAbove: true },   // branch below, labels above
    { id: 'st4', th: 240, no: '05', name: 'EXECUTE + OBSERVE', plain: 'act inside a wall',
      lx: 38, ly: -20, la: 'start',  pAbove: true },   // sandbox left, labels interior
    { id: 'st5', th: 300, no: '06', name: 'COMPACT + VERIFY',  plain: 'forget on purpose',
  ...
  var REGIONS = [
    ...
    { id: 'run',  w: [0.520, 0.760], no: '»',  lab: 'RUN',      state: 'live // fix the flaky retry test' },
    { id: 'ret',  w: [0.760, 1.001], no: '✓',  lab: 'RETURN',   state: 'returned in 5 turns' }
  ];
  ...
  tbt.textContent = 'LOOP ENGINEERING · REV 05 · SHEET 1 OF 1';
  ...
  var plateEls = [];
  [
    [676, 78, 260, 190, 1.45],    // S1 feeders + labels below
    [838, 328, 274, 182, 1.45],   // S2 prompt stack
  ...
  (function () { // S1: feeders
    ...
    label('files', 730, 140, { dx: -8, dy: -8, a: 'end', fs: 12, lod: 1.45 }, g);
    label('memory', 800, 118, { dy: -14, a: 'middle', fs: 12, lod: 1.45 }, g);
    label('tools', 870, 140, { dx: 8, dy: -8, fs: 12, lod: 1.45 }, g);
  }());
  (function () { // S2: the prompt stack, inside the ring so it owns clear space
    var g = intGroup('st1');
    el('line', { x1: 1046, y1: 372, x2: 995, y2: 396 }, g);
    ['system', 'history', 'knowledge', 'ask'].forEach(function (n, i) {
  ...
  (function () { // S3: the rented black box
    ...
    label('f(prompt) → tokens', 1160, 626, { dx: 12, fs: 12, lod: 1.45 }, g);
    label('temp 0.2 // stop ¶', 1098, 588, { dy: -4, fs: 11.5, lod: 1.45 }, g);
  }());
  (function () { // S4: the router
    var g = intGroup('st3');
    el('line', { x1: 800, y1: 824, x2: 770, y2: 852 }, g);
    el('line', { x1: 800, y1: 824, x2: 830, y2: 852 }, g);
    label('text', 770, 852, { dx: -8, dy: 14, a: 'end', fs: 12, lod: 1.45 }, g);
    label('tool_call', 830, 852, { dx: 8, dy: 14, fs: 12, lod: 1.45 }, g);
  }());
  (function () { // S5: the sandbox
    ...
    label('sandbox.run(act)', 430, 598, { dy: -4, fs: 11.5, lod: 1.45 }, g);
    label('observation', 434, 662, { dy: 20, fs: 11.5, lod: 1.45 }, g);
  }());
  (function () { // S6: the funnel
    ...
    label('summarize()', 420, 278, { dy: -6, fs: 11.5, lod: 1.45 }, g);
    label('62k → 31k', 420, 352, { dy: 20, fs: 11.5, lod: 1.45 }, g);
  }());
  ...
  function render() {
    ...
    if (ri !== lastRegion) {
      lastRegion = ri;
      mastState.textContent = ri === -1 ? 'an engraving of an agent' : REGIONS[ri].state;
    ...
    else if (p >= 0.392 && p < 0.455) { vTxt = 'pass'; vCls = 'v-pass'; }
